ARG S6_ARCH
FROM oznu/s6-node:14.17.0-${S6_ARCH:-amd64}

RUN apk add --no-cache --virtual .ffmpeg-build \
  git python2 python3 make g++ avahi-compat-libdns_sd avahi-dev dbus iputils sudo nano \
  && chmod 4755 /bin/ping \
  && mkdir /homebridge \
